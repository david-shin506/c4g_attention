	...
	const float low_pass)
{
  ...
  return std::make_tuple(rendered, out_color, out_feature_map, radii, geomBuffer, binningBuffer, imgBuffer, out_depth, out_opaticy, n_touched); /***/
}
	...
	const float low_pass) 
{
  ...
  torch::Tensor dL_dsemantic_feature = torch::zeros({P, NUM_SEMANTIC_CHANNELS}, means3D.options()); /***/ 
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dsemantic_feature, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dtau); /***/
}
