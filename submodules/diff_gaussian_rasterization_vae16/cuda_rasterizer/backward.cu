...
)
{
	...
	// __shared__ float dL_dsemantic_feature_shared[NUM_SEMANTIC_CHANNELS * BLOCK_SIZE]; /***/
	...
	// float last_semantic_feature[NUM_SEMANTIC_CHANNELS] = { 0.f };
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
			...
			// float local_dL_dfeature[NUM_SEMANTIC_CHANNELS];
			#pragma unroll
			for (int ch = 0; ch < NUM_SEMANTIC_CHANNELS; ch++)
			{
				// const float f = collected_semantic_feature[ch * BLOCK_SIZE + j]; 			// let me check
				...
				// local_dL_dfeature[ch] = skip ? 0.0f : dchannel_dsemantic_feature * dL_dfeaturechannel;
				atomicAdd(&(dL_dsemantic_feature[global_id * NUM_SEMANTIC_CHANNELS + ch]), skip ? 0.0f : dchannel_dsemantic_feature * dL_dfeaturepixels[ch * H * W + pix_id]); 
			}
