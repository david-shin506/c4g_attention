...
__device__ float3 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix, float low_pass)
{
	...
	const float low_pass)
{
	...
	float3 cov = computeCov2D(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, cov3D, viewmatrix, low_pass);
	...
	if (!isfinite(my_radius) || !isfinite(point_image.x) || !isfinite(point_image.y))
		return;
	const float stored_radius = fminf(my_radius, 2147483520.0f);
