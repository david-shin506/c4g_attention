...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image", "framewise_decoding", "context_video", "rendered_video", "context_idx_orig"),
            output_params=("latents", "input_latents", "context_latents", "rendered_latents"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image, framewise_decoding, context_video=None, rendered_video=None, context_idx_orig=None):
        ...
        elif context_idx_orig is not None:
            # view1 (keyframes): encode frame-by-frame (1 latent per frame)
            # view2 (4k+1 padded): encode as chunk → (T_view2-1)/4+1 latents
            T_v1 = len(context_idx_orig)
            lat1 = torch.concat([
                pipe.vae.encode(input_video[:, :, i:i+1], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
                for i in range(T_v1)
            ], dim=2)
            lat2 = pipe.vae.encode(input_video[:, :, T_v1:], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
            input_latents = torch.concat([lat1, lat2], dim=2)
        else:
            T_total = input_video.shape[2]
            T_ctx = T_total // 2
            if T_ctx * 2 == T_total:
                # Equal-length fallback (e.g., kubric 25+25)
                lat1 = pipe.vae.encode(input_video[:, :, :T_ctx], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
                lat2 = pipe.vae.encode(input_video[:, :, T_ctx:], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
                input_latents = torch.concat([lat1, lat2], dim=2)
            else:
        ...
        if context_video is not None:
            ...
            context_latents = pipe.vae.encode(context_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)

        rendered_latents = None
        if rendered_video is not None:
            rendered_video = pipe.preprocess_video(rendered_video)
            rendered_latents = pipe.vae.encode(rendered_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)

        # print(f"[DIM] InputVideoEmbedder: training={pipe.scheduler.training}")
        # print(f"[DIM]   noise.shape={noise.shape}")
        # print(f"[DIM]   input_latents.shape={input_latents.shape}")
        # if context_latents is not None:
        #     print(f"[DIM]   context_latents.shape={context_latents.shape}")
        # if rendered_latents is not None:
        #     print(f"[DIM]   rendered_latents.shape={rendered_latents.shape}")

        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents, "context_latents": context_latents, "rendered_latents": rendered_latents}
        else:
            # Align noise T to rendered_latents T to avoid shape mismatch in denoising loop
            if rendered_latents is not None:
                T_r = rendered_latents.shape[2]
                noise = noise[:, :, :T_r]
                input_latents_aligned = input_latents[:, :, :T_r]
                # print(f"[DIM]   noise truncated to T={T_r} to match rendered_latents")
            else:
                input_latents_aligned = input_latents
            latents = pipe.scheduler.add_noise(input_latents_aligned, noise, timestep=pipe.scheduler.timesteps[0])
            # print(f"[DIM]   latents.shape={latents.shape}")
            return {"latents": latents, "context_latents": context_latents, "rendered_latents": rendered_latents}
...
class WanVideoUnit_VACE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("vace_video", "vace_video_mask", "vace_reference_image", "vace_scale", "height", "width", "num_frames", "tiled", "tile_size", "tile_stride", "vace_video_idx"),
            output_params=("vace_context", "vace_scale", "vace_video_idx"),
            onload_model_names=("vae",)
        )
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            b, c, t, h, w = vace_video.shape

            if vace_video_mask is None:
                vace_video_mask = torch.ones_like(vace_video)
            else:
                if len(vace_video_mask.shape) == 1:
                    vace_video_mask = vace_video_mask.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                    vace_video_mask = vace_video_mask.repeat(1, c, h, w)
                vace_video_mask = pipe.preprocess_video(vace_video_mask, min_value=0, max_value=1)

            # Derive split point from mask: first frame where mask == 1
            _mask_t = vace_video_mask[0, 0, :, 0, 0]  # (T,)
            _ones = (_mask_t > 0.5).nonzero(as_tuple=True)[0]
            T_ctx = int(_ones[0].item()) if len(_ones) > 0 else vace_video.shape[2] // 2

            ref_video = vace_video[:, :, :T_ctx]
            ref_mask = vace_video_mask[:, :, :T_ctx]

            ren_video = vace_video[:, :, T_ctx:]
            ren_mask = vace_video_mask[:, :, T_ctx:]
            
            ref_inactive = ref_video * (1 - ref_mask) + 0 * ref_mask
            ref_reactive = ref_video * ref_mask + 0 * (1 - ref_mask)

            ren_inactive = ren_video * (1 - ren_mask) + 0 * ren_mask
            ren_reactive = ren_video * ren_mask + 0 * (1 - ren_mask)
            
            # view1 (ref) = keyframes → frame-by-frame VAE (1 latent per frame)
            def _enc_per_frame(vid):
                T_v = vid.shape[2]
                return torch.concat([
                    pipe.vae.encode(vid[:, :, i:i+1], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
                    for i in range(T_v)
                ], dim=2)
            ref_inactive = _enc_per_frame(ref_inactive)  # (1, 16, N_keyframes, H', W')
            ref_reactive = _enc_per_frame(ref_reactive)
            # view2 (ren) = 4k+1 frames → chunk VAE
            ren_inactive = pipe.vae.encode(ren_inactive, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
            ren_reactive = pipe.vae.encode(ren_reactive, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
            ...
            T_lat_ref = ref_inactive.shape[2]  # = N_keyframes
            T_lat_ren = ren_inactive.shape[2]  # = (T_view2-1)/4+1
            H_lat, W_lat = ref_inactive.shape[3], ref_inactive.shape[4]
            # vace_in_dim=96: 32 (latent) + 64 (mask)
            vace_video_latent_ref = torch.concat((ref_inactive, ref_reactive), dim=1)
            vace_video_latent_ren = torch.concat((ren_inactive, ren_reactive), dim=1)

            vace_mask_latents_ref = rearrange(ref_mask[0,0], "T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8)
            vace_mask_latents_ren = rearrange(ren_mask[0,0], "T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8)
            vace_mask_latents_ref = torch.nn.functional.interpolate(vace_mask_latents_ref, size=(T_lat_ref, H_lat, W_lat), mode='nearest-exact')
            vace_mask_latents_ren = torch.nn.functional.interpolate(vace_mask_latents_ren, size=(T_lat_ren, H_lat, W_lat), mode='nearest-exact')
            ...
            vace_context_ref = torch.concat((vace_video_latent_ref, vace_mask_latents_ref), dim=1)
            vace_context_ren = torch.concat((vace_video_latent_ren, vace_mask_latents_ren), dim=1)
            vace_context = torch.concat((vace_context_ref, vace_context_ren), dim=2)
            return {"vace_context": vace_context, "vace_scale": vace_scale, "vace_video_idx": vace_video_idx}
        else:
            return {"vace_context": None, "vace_scale": vace_scale, "vace_video_idx": vace_video_idx}
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # C4G: context_tokens prepend using patch_embedding_context (Conv3d(16, dim))
    n_context_tokens = 0
    if context_latents is not None and hasattr(dit, 'patch_embedding_context'):
        context_tokens = dit.patch_embedding_context(context_latents)  # (B, dim, f, h, w)
        fc = context_tokens.shape[2]
        n_context_tokens = fc * h * w
        context_tokens = rearrange(context_tokens, 'b c f h w -> b (f h w) c').contiguous()
        print(f"[SHAPE] token-concat: context_tokens={context_tokens.shape} + x_tokens={x.shape}")
        x = torch.cat([context_tokens, x], dim=1)  # prepend context tokens
        print(f"[SHAPE] token-concat result: x={x.shape}")
    ...
    if vace_context is not None and vace_video_idx is not None:
        f_vace = vace_context.shape[2]
        ref_len = (1-kwargs['vace_video_mask']).sum().int().item()
        lat_indices_ref = vace_video_idx[:ref_len]                              # ref: 1:1 (per-frame encode)
        lat_indices_ren = torch.arange(f_vace - ref_len, device=x.device)       # ren: 1x
        lat_indices = torch.cat([lat_indices_ref, lat_indices_ren], dim=0).cpu()


        freqs = torch.cat([
            dit.freqs[0][lat_indices].view(f_vace, 1, 1, -1).expand(f_vace, h, w, -1),
            dit.freqs[1][:h].view(1, h, 1, -1).expand(f_vace, h, w, -1),
            dit.freqs[2][:w].view(1, 1, w, -1).expand(f_vace, h, w, -1),
        ], dim=-1).reshape(f_vace * h * w, 1, -1).to(x.device)
    else:
        freqs = torch.cat([
    ...
    # C4G: context_tokens share the same timestamps as x_tokens → prepend same freqs
    if n_context_tokens > 0:
        freqs = torch.cat([freqs[:n_context_tokens], freqs], dim=0)
    ...
    if vace_context is not None:
        if vace_video_idx is not None:
            # vace_video_idx: pixel-space timestamps for each frame in vace_video
            # latent frame i → pixel frame i*4 → look up vace_video_idx[i*4]
            f_vace = vace_context.shape[2]
            ref_len = (1-kwargs['vace_video_mask']).sum().int().item()
            lat_indices_ref = vace_video_idx[:ref_len]
            lat_indices_ren = torch.arange(f_vace - ref_len, device=x.device)
            
            lat_indices = torch.cat([lat_indices_ref, lat_indices_ren], dim=0).cpu()
            # temporal_positions = vace_video_idx[lat_indices].long().cpu()
            # vace c is zero-padded to f*h*w tokens (DiT x token count), so vace_freqs must match
            vace_real = torch.cat([
                dit.freqs[0][lat_indices].view(f_vace, 1, 1, -1).expand(f_vace, h, w, -1),
                dit.freqs[1][:h].view(1, h, 1, -1).expand(f_vace, h, w, -1),
                dit.freqs[2][:w].view(1, 1, w, -1).expand(f_vace, h, w, -1),
            ], dim=-1).reshape(f_vace * h * w, 1, -1)
            pad_f = f - f_vace
            if pad_f > 0:
                vace_pad = torch.cat([
                    dit.freqs[0][f_vace:f].view(pad_f, 1, 1, -1).expand(pad_f, h, w, -1),
                    dit.freqs[1][:h].view(1, h, 1, -1).expand(pad_f, h, w, -1),
                    dit.freqs[2][:w].view(1, 1, w, -1).expand(pad_f, h, w, -1),
                ], dim=-1).reshape(pad_f * h * w, 1, -1)
                vace_freqs = torch.cat([vace_real, vace_pad], dim=0).to(x.device)
            else:
                vace_freqs = vace_real.to(x.device)
        else:
            vace_freqs = freqs
        vace_hints = vace(
            x, vace_context, context, t_mod, vace_freqs,
            use_gradient_checkpointing=use_gradient_checkpointing,
            use_gradient_checkpointing_offload=use_gradient_checkpointing_offload
        )
    if tea_cache_update:
    ...
    if reference_latents is not None:
        ...
        f -= 1
    # C4G: unpatchify full sequence (context_tokens + x_tokens)
    # context_tokens count is prepended at the front: n_context_tokens tokens
    f_total = f + (n_context_tokens // (h * w) if n_context_tokens > 0 else 0)
    x = dit.unpatchify(x, (f_total, h, w))
    return x
