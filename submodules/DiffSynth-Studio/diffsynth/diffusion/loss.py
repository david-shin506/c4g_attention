...
def FlowMatchSFTLoss(pipe: BasePipeline, **inputs):
    ...
    # Optionally restrict loss to a temporal slice (e.g. view2 only)
    if inputs.get("loss_latent_start") is not None:
        T_start = inputs["loss_latent_start"]
        noise_pred      = noise_pred[:, :, T_start:]
        training_target = training_target[:, :, T_start:]
