...
class ViewSamplerBoundedCfg:
    ...
    start_index_initial: int = 0


class ViewSamplerBounded(ViewSampler[ViewSamplerBoundedCfg]):
    def inv_schedule(self, initial: int, final: int) -> int:
        fraction = self.global_step / self.cfg.warm_up_steps
        return max(initial - int((initial - final) * fraction), final)
    ...
    def sample(
        ...
        if self.stage == "test":
            ...
            start_index = self.inv_schedule(
                self.cfg.start_index_initial,
                0,
            )
        else:
            max_gap = self.cfg.max_distance_between_context_views
        ...
        index_context_left = torch.randint(
            num_views - start_index if self.cameras_are_circular else num_views - context_gap - start_index,
            size=tuple(),
            device=device,
        ).item()
        if self.stage == "test":
        ...
        if self.cfg.num_context_views > 2:
            ...
            extra_views = []
            while len(extra_views) != num_extra_views:
