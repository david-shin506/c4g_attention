...
def visualize_attention_map(
    ...
    query = F.normalize(qkv, dim=-1)
    key = F.normalize(qkv, dim=-1)
    
    attn_score = torch.einsum('bid,bjd->bij', query, key)  # (B, num_heads, N, N)
    ...
    attn_score = attn_score[:, guidance_feature_size + gaussian_token_idx, : guidance_feature_size]
