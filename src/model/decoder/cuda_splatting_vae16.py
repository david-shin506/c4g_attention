"""HG feature rendering adapter, isolated for 16-channel Wan latents."""
...
from submodules.diff_gaussian_rasterization_vae16.diff_gaussian_rasterization import (
...
def render_cuda(
    ...
    gaussian_features: Float[Tensor, "batch gaussian feature_dim"] | None = None,
    scale_invariant: bool = True,
    use_sh: bool = True,
    cam_rot_delta: Float[Tensor, "batch 3"] | None = None,
    cam_trans_delta: Float[Tensor, "batch 3"] | None = None,
    low_pass_filter: float = 0.3,
) -> tuple[Float[Tensor, "batch 3 height width"], Float[Tensor, "batch height width"], Float[Tensor, "batch feature_dim height width"] | None]:
    ...
    all_depths = []
    all_features = []
    use_feature_rasterizer = gaussian_features is not None
    for i in range(b):
    ...
    feature_out = torch.stack(all_features) if use_feature_rasterizer else None
    return torch.stack(all_images), torch.stack(all_depths), feature_out
