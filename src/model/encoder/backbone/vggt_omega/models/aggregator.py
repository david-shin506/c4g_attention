...
class Aggregator(nn.Module):
    ...
    def forward(
        ...
        def embed_images(input_images: torch.Tensor) -> torch.Tensor:
            embedded = self.patch_embed(input_images)
            if isinstance(embedded, dict):
                embedded = embedded["x_norm_patchtokens"]
            return embedded

        if self.gradient_checkpoint and self.training and torch.is_grad_enabled():
            patch_tokens = checkpoint_fn(embed_images, images, use_reentrant=False)
        else:
            patch_tokens = embed_images(images)
        ...
        for block_idx in range(self.depth):
            def run_layer(
                ...
                current_block_idx: int = block_idx,
            ) -> tuple[torch.Tensor, torch.Tensor]:
                output_tokens, output_frame_tokens = self._run_frame_block(
                    input_tokens,
                    batch_size,
                    num_frames,
                    num_tokens,
                ...
                output_tokens = self._run_inter_frame_attention_block(
                    output_tokens,
                    batch_size,
                    num_frames,
                    num_tokens,
                    embed_dim,
                    current_block_idx,
                    self.inter_frame_attention_types[current_block_idx],
                )
                return output_tokens, output_frame_tokens

            if self.gradient_checkpoint and self.training and torch.is_grad_enabled():
                tokens, frame_tokens = checkpoint_fn(
                    run_layer,
                    tokens,
                    use_reentrant=False,
                )
            else:
                tokens, frame_tokens = run_layer(tokens)
